...
In DEAP-3600, photons are absorbed by a wavelength shifting medium and re-emitted in an unpredictable direction, before being detected by one of 255 photomultiplier tubes (PMTs) on the surface of the spherical detector. The randomness severely limits the accuracy of conventional classifiers; the best so far removes 99.6\% of alpha background radiation, while also (undesirably) removing 91.0\% of WIMP events. Because of physical limitations, simulated data is used for calibration, with 30 real-world experimental events available for testing.
