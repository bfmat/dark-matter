...
\begin{figure}[ht]
    \centering
    \subfloat{\fbox{\includegraphics[width=0.8\textwidth]{audio_cropped}}}
    \qquad
...
\begin{enumerate}
    ...
    \item I developed and applied a Mercator-like cylindrical projection, by mapping each row of nodes in the sphere to a row in a rectangular image. Cubic spline interpolation was used to map latitude lines on the sphere (which vary in length) to equal-length rows on the rectangular image.
...
\begin{figure}[ht]
    \centering
    \fbox{\includegraphics[width=0.85\textwidth]{pico_final_results}}
