...
# Convert the AP values to binary predictions as to whether the event is loud or quiet
ap_predictions = np.array(ap_values) > 1.1
# Get an array of the ground truths for this data
ground_truths = np.concatenate([np.ones(len(loud_events)), np.zeros(len(quiet_events))])
# Calculate how often they agree (how many examples AP is correct on)
agreements = np.count_nonzero(ap_predictions == ground_truths)
# Calculate and print the resulting accuracy value
print('AP accuracy:', agreements / len(all_events))
