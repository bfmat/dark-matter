...
HYPERPARAMETERS = ['dropout', 'l2_lambda', 'initial_threshold', 'threshold_multiplier', 'initial_examples']
...
for configuration_index, configuration in enumerate(configurations):
    ...
    for hyperparameter_index, hyperparameter in enumerate(HYPERPARAMETERS):
        if hyperparameter == 'initial_examples' and float(configuration.split(hyperparameter)[1].split('/')[0]) == 256:
