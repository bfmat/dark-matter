    ...
    def __init__(self, csv_path: Optional[str] = None, values: Optional[List[List[float]]] = None, positions: Optional[List[Tuple[float]]] = None, ground_truths: Optional[List[bool]] = None) -> None:
        ...
        for node in self.nodes:
            ...
            connected_nodes_clockwise = [
                ...
                # Let -1 pass through; it represents no connection at a position
                if node_identifier != -1 else -1
                # The identifiers are in the original order
                for node_identifier in node.raw_connections_clockwise
            ]
            # Get the Y (vertical) positions of these connected nodes, replacing -1 with negative infinity (so empty connections will never be considered the farthest up)
            y_positions = [connected_node.y_position if connected_node != -1 else float('-inf') for connected_node in connected_nodes_clockwise]
            ...
            node.connections = [node.raw_connections_clockwise[(highest_node_index + index_offset) % len(node.raw_connections_clockwise)]
